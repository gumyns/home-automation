...
int main() {
	boost::filesystem::path path("../lib");
 	auto factory = boost::dll::import<gumyns::sh::plugin::device::ParserFactory>(
 		path / "SensorWindow", "pDevice", boost::dll::load_mode::append_decorations
 	);
	...
	nlohmann::json readMultiple = 			nlohmann::json::parse("{\"r\":[\"status\", \"status0\"]}");
	...
	/*
	 * !! NOTE: All cases should return same request, as it's sensor! !!
	 * Request : [20]
	 * Response : [60 xx xx xx xx]
	 */
	...
	parseFrom(parser.get(), {0x03 << 5, 0x00});
	parseFrom(parser.get(), {0x03 << 5, 0x03});
	parseFrom(parser.get(), {0x03 << 5, 0x01});
	parseFrom(parser.get(), {0x03 << 5, 0x0f});
